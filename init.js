...
/**
 * Custom implementation of setTimeout to work with the Java hMod JSApi Plugin
 */
var SetTimeoutCustom = false;
var SetTimeoutCallbacks = {};

var SetTimeoutRunTimers = function() {
	if(SetTimeoutCustom) {
		//Api.log.info("Timers being looked at.");
		var timersToExecute = [];
		var curTime = new Date().getTime();
		for (var tk in SetTimeoutCallbacks) {
			if (SetTimeoutCallbacks.hasOwnProperty(tk) && SetTimeoutCallbacks[tk].shouldExecute(curTime)) {
				timersToExecute.push(SetTimeoutCallbacks[tk]);
			}
		}

		for (var i in timersToExecute) {
			if (timersToExecute.hasOwnProperty(i)) {
				//Api.log.info("Timer executing.");
...
//if(typeof(setTimeout) != "function") {
	...
	var SingleTimeoutInstance = function(fptr, timeMs) {
		this.uid = setTimeout.uidGenerator++;
		this.fptr = fptr;
		this.interval = timeMs;
		this.requestedTime = new Date().getTime() + timeMs;
	};

	SingleTimeoutInstance.prototype.shouldExecute = function(curTime) {
	...
	SingleTimeoutInstance.prototype.execute = function() {
		delete SetTimeoutCallbacks[this.uid];
		this.fptr();
	};
	...
	setTimeout = function(fptr,requestedTime) {
		...
		var inst = new SingleTimeoutInstance(fptr,requestedTime);
		SetTimeoutCallbacks[inst.uid] = inst;
		return inst.uid;
	};
	clearTimeout = function(uid) {
		if(SetTimeoutCallbacks.hasOwnProperty(uid)) {
			delete SetTimeoutCallbacks[uid];
		}
	};
	...
	Api.onLoginChecks(SetTimeoutRunTimers);
	Api.onLogin(SetTimeoutRunTimers);
	Api.onDisconnect(SetTimeoutRunTimers);
	Api.onChat(SetTimeoutRunTimers);
	Api.onCommand(SetTimeoutRunTimers);
//	Api.onConsoleCommand(SetTimeoutRunTimers);
	Api.onBan(SetTimeoutRunTimers);
	Api.onIpBan(SetTimeoutRunTimers);
	Api.onKick(SetTimeoutRunTimers);
	Api.onBlockCreate(SetTimeoutRunTimers);
	Api.onBlockDestroy(SetTimeoutRunTimers);
	Api.onArmSwing(SetTimeoutRunTimers);
	Api.onInventoryChange(SetTimeoutRunTimers);
	Api.onCraftInventoryChange(SetTimeoutRunTimers);
	Api.onEquipmentChange(SetTimeoutRunTimers);
	Api.onComplexBlockChange(SetTimeoutRunTimers);
